...
Let's go through the document in detail:
...
https://github.com/inspire-eu-validation/ets-repository/blob/master/inspire-bsxets.xq[inspire-bsxets.xq];
* ... test features (elements substitutable for gml:AbstractFeature),
but without using the GmlGeoX library, use
https://github.com/inspire-eu-validation/ets-repository/blob/master/inspire-noggeo-bsxets.xq[inspire-noggeo-bsxets.xq];
* ... test metadata records (gmd:MD_Metadata), always use
https://github.com/inspire-eu-validation/ets-repository/blob/master/inspire-md-bsxets.xq[inspire-md-bsxets.xq].

The generic template included with ETF is designed for testing features
...
Currently only one Test Object Type is supported and statically
predefined by the Test Driver:

[source,XML]
----
<etf:TestObjectType xmlns="http://www.interactive-instruments.de/etf/2.0"
...
* If a test suite fails then all dependent test suites will be skipped.
For example, if test suite B fails, test suite A will be skipped.
...
that is consistent with the dependecies.

Here are examples how dependencies are declared in XML in an
`etf:ExecutableTestSuite` document. For test suites:
...
and the links to them are included in the `etf:TestTaskResult`. An
example:

[source,XML]
----
<attachments>
 <Attachment type="LogFile" id="EID784dc01e-280d-4f18-886b-2a024be076b2">
    <label>Log file</label>
    <encoding>UTF-8</encoding>
    <mimeType>text/plain</mimeType>
    <referencedData href="file:///Users/portele/Documents/Dropbox/ETF/ets-repository/inspire/tmp/EID2fbe9703-aa7e-4cf3-b525-d8292aa84988-log.txt"/>
 </Attachment>
 <Attachment type="StatisticalReport" id="EID5083f179-3dfa-42b5-a764-2c9abeb9111f">
    <label>Feature statistics</label>
    <encoding>UTF-8</encoding>
    <mimeType>application/xml</mimeType>
    <referencedData href="file:///Users/portele/Documents/Dropbox/ETF/ets-repository/inspire/tmp/EID2fbe9703-aa7e-4cf3-b525-d8292aa84988-stat.xml"/>
 </Attachment>
 <Attachment type="Query" id="EIDc4b1bbb6-27c1-4f1f-860a-352441da4296">
    <label>XQuery executed against the dataset</label>
    <encoding>UTF-8</encoding>
    <mimeType>text/plain</mimeType>
    <referencedData href="file:///Users/portele/Documents/Dropbox/ETF/ets-repository/inspire/tmp/EID2fbe9703-aa7e-4cf3-b525-d8292aa84988-query.xq"/>
 </Attachment>
</attachments>
----
