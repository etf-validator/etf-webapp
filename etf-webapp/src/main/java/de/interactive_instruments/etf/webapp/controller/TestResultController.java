...
public class TestResultController {
    ...
    public void init() throws IOException, TransformerConfigurationException,
        ...
        final IFile reportDir = etfConfig.getPropertyAsFile(EtfConstants.ETF_INTERNAL_DATABASE_DIR).expandPath("obj");
        ...
        final long exp = etfConfig.getPropertyAsLong(EtfConfig.ETF_TESTREPORTS_LIFETIME_EXPIRATION);
        if (exp > 0) {
    ...
    @ApiOperation(value = "Get a single Test Result as CSV", notes = TEST_RUN_DESCRIPTION, tags = {TEST_RESULTS_TAG_NAME})
    @RequestMapping(value = {TEST_RUNS_URL + "/{id}.csv"}, method = RequestMethod.GET)
    public void testRunByIdCsv(
            @ApiParam(value = "Test Run ID. "
                    + EID_DESCRIPTION, example = EID_EXAMPLE, required = true) @PathVariable String id,
            HttpServletRequest request, HttpServletResponse response)
            throws IOException, ObjectWithIdNotFoundException {
        final PreparedDto<TestRunDto> preparedDto = this.testRunDao.getById(EidConverter.toEid(id));
        setMaxAgeHeader(response);
        response.setContentType("text/csv");
        response.setHeader("Content-Disposition",
                "attachment; filename=\"" + IFile.sanitize(preparedDto.getDto().getLabel()) + ".csv\"");
        final OutputFormat csvOutputFormat = this.testRunDao.getOutputFormats()
                .get(EidFactory.getDefault().createUUID(
                        this.testRunDao.getDtoType().getSimpleName() + "DsResult2Csv"));
        preparedDto.streamTo(csvOutputFormat, null, response.getOutputStream());
    }
    ...
    @ApiOperation(value = "Get a Test Run's log by ID", notes = "Retrieves all messages that were logged during a Test Run.", tags = {
            TEST_RESULTS_TAG_NAME})
    @RequestMapping(value = {TEST_RUNS_URL + "/{id}.log", TEST_RUNS_URL + "/{id}/log"}, method = RequestMethod.GET)
